...
#Set folders
files <- here("data/field_deployment/raw")
salfiles <- here('data/field_deployment/correctedSalinity')
ofiles <- here('data/field_deployment/correctedO2')
parfiles <- here('data/field_deployment/correctedPAR')
...
## Other option
depth_dat2 <- depth %>%
  map(~ read_csv(file.path(here("data/field_deployment/raw"), .),skip = 2, col_names = c("datetime","depth"), col_types = "_?__?"))
...
output <- here("data/metabolism_objects")
...
for(i in 1:length(names)){
  ...
  write_csv(dftest[[i]], file.path(output, paste0(names[i],"_metab_data.csv"))) # write csv
  
  saveRDS(dftest[[i]], file.path(output, paste0(names[i],"_metab_data.rds"))) # save rds
